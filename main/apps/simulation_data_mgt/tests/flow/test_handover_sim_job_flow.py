...
class HandoverSimJobFlowTestCase(TestCase):
    def test_full_flow_of_handover_sim_job(self):
        ...
        # 2) create_handover_sim_job
        create_url = reverse("create_handover_sim_job")
        create_data = {
            "handover_sim_job_name": "Project Flow handover_sim_job",
            "handover_sim_job_parameter": {
                "some_key": "some_value"
            },
            "f_user_uid": user_uid
        }
        resp = self.client.post(create_url, data=json.dumps(create_data), content_type="application/json")
        self.assertIn(resp.status_code, [200, 400], "create_handover_sim_job => 200 or 400")
        resp_json = resp.json()
        sim_job_uid = resp_json.get("data", {}).get("handover_sim_job_uid")
        print("=== create_handover_sim_job =>", sim_job_uid, "===")

        # 3) run_handover_sim_job
        run_url = reverse("run_handover_sim_job")
        run_data = {
            "handover_sim_job_uid": sim_job_uid
        }
        ...
        print("=== download_handover_sim_result =>", resp.status_code, resp.json() if resp.status_code in [400,404] else 'PDF???', "===")
        ...
        print("=== delete_handover_sim_result =>", resp.status_code, resp.json().get("status"), "===")

        # 6) delete_handover_sim_job
        delete_sim_job_url = reverse("delete_handover_sim_job")
        del_job_data = {
            "handover_sim_job_uid": sim_job_uid
        }
        resp = self.client.post(delete_sim_job_url, data=json.dumps(del_job_data), content_type="application/json")
        self.assertIn(resp.status_code, [200, 400], "delete_handover_sim_job => 200 or 400")
        print("=== delete_handover_sim_job =>", resp.status_code, resp.json().get("status"), "===")
