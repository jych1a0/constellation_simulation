...
@log_trigger('INFO')
def genGsoResultPDF(gso):
    # 獲取當前時間
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    
    # 解析模擬結果 (從字串轉換為字典)
    sim_results = gso.gso_simulation_result['gso_simulation_result']
    ...
    pdf_path = os.path.join(gso.gso_data_path, 'gso_simulation_report.pdf')  # 假設儲存在當前目錄
    # print(pdf_path)
    pdf_pages = PdfPages(pdf_path)
    base_dir = os.path.dirname(os.path.abspath(__file__))  
    config_path = os.path.join(base_dir, "update_parameter", "dynamic_config.json")
    gso.gso_parameter = update_parameter(
        gso.gso_parameter,
        config_path=config_path,
        process_name="gso_process"
    )
    try:
        # 第一頁：實驗條件
        ...
        condition_data = [[k, str(v).replace('TLE_6P_22Sats_29deg_F1', '6 * 22')
                 ...
                 for k, v in gso.gso_parameter.items()]
        ...
        # # 第二頁：Cell Topology
        # cell_ut = gso.gso_parameter.get('cell_ut', '')
