...
def genMultiToMultiResultPDF(multiToMulti):
    """
    讀取 multiToMulti 的模擬結果(multiToMulti.multiToMulti_simulation_result)，
    產生 PDF 報告並返回 PDF 路徑。
    """
    # 取得當前時間（做為報表生成時間）
    ...
    # 取得模擬結果（字典）
    sim_results = multiToMulti.multiToMulti_simulation_result
    ...
    try:
        ...
        # 如果 multiToMulti 中有參數設定 (multiToMulti.multiToMulti_parameter)，可在此顯示
        condition_data = [
            [k, str(v)] for k, v in multiToMulti.multiToMulti_parameter.items()
        ]
        ...
        plt.title('MultiToMulti Simulation Conditions', pad=20, size=14, weight='bold')
        plt.tight_layout()
        ...
        plt.title('MultiToMulti Simulation Results', pad=20, size=14, weight='bold')
        plt.tight_layout()
